...
# GARANTE A INSTALAÇÃO DO COMPOSER E DEPENDÊNCIAS
COPY --from=composer:2 /usr/bin/composer /usr/bin/composer
RUN composer install --no-dev --optimize-autoloader --no-interaction

# LIMPEZA DE CACHE QUE PODE TER VINDO DO GITHUB
RUN rm -rf bootstrap/cache/*.php storage/framework/cache/data/*

# INJEÇÃO DA CHAVE DIRETAMENTE NO ARQUIVO DE CONFIGURAÇÃO
...
RUN echo 'php artisan migrate --force > /dev/null 2>&1 || echo "Banco pronto."' >> /usr/local/bin/start.sh
